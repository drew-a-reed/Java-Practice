...
/*		if ((nums[0] == 6) || (nums[nums.length - 1] == 6)) {
	...
	public boolean sameFirstLast(int[] nums) {
		boolean returnedArrayLength = nums.length > 1;
		boolean returnedArrayEqual = (nums[0] == nums[nums.length-1]);

		return returnedArrayEqual && returnedArrayLength;
	/*	if (returnedArrayEqual && returnedArrayLength) {
			return true;
		}
		return false;
	}*/
	...
	public int sum13(int[] nums) {
		if (nums.length > 0) {
			int sum = 0;
			for (int i = 0; i < nums.length; i++) {   // for (int num : nums) {sum = sum + num;}
				if (nums[i] == 13){
					i++;
					continue;
				}
