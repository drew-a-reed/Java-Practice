...
public class Exercise04_HotelReservation {
    ...
    they've raised the rate for weekend night rates to $99.99 regardless of how many nights a guest
    is staying. If a guest is staying 3 or more nights, the weekday rate is still $89.99 per night.
    Otherwise, the $99.99 rate applies. Implement the logic to return the total amount of the stay
    based on the total number of nights and the number of weekend nights.
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        ...
        if (numOfWeekendNights > 0 && numOfTotalNights >= 3) {
            return calculateStayTotal(numOfTotalNights) - (numOfWeekendNights * DISCOUNT_RATE) +
                    (numOfWeekendNights * DAILY_RATE);
        } else
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights, boolean isRewardsMember) {
        ...
        if (isRewardsMember) {
           //System.out.println(calculateStayTotal(numOfTotalNights));
