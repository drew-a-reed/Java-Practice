...
public class Exercise01_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        ...
        int standRatePounds = weightPounds;
        int additionalPounds = 0;

        if (weightPounds > MAX_WEIGHT_POUNDS) {
            standRatePounds = MAX_WEIGHT_POUNDS;
            additionalPounds = weightPounds - 40;
        }

        double standardCost = standRatePounds * UP_TO_40_LB_RATE;
        double additionalCost = additionalPounds * OVER_40_LB_RATE;
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        if (discountPercentage > 0) {
            double shippingCost = calculateShippingTotal(weightPounds);
            //System.out.println(shippingCost);
            double discountPrice = shippingCost * (1- discountPercentage);
